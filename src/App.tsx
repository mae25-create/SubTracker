...
import { Plus, LayoutGrid, List, AlertCircle, Filter, Crown, Sun, Moon, CalendarDays, Check, Trash2 } from 'lucide-react';
...
export default function App() {
  ...
  const handleBulkDelete = () => {
    if (window.confirm(`Are you sure you want to delete ${selectedIds.length} subscriptions?`)) {
  ...
  const handleSelectAll = () => {
    if (selectedIds.length === filteredSubscriptions.length && filteredSubscriptions.length > 0) {
      setSelectedIds([]);
    } else {
  ...
  return (
    <div className="min-h-screen pb-20 selection:bg-amber-500/30">
        ...
        <div className="flex flex-col gap-4 mb-10">
          {expiredSubs.length > 0 && (
            <div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 rounded-2xl p-5 flex items-start gap-4 shadow-lg shadow-red-500/5 backdrop-blur-sm transition-colors duration-300">
              <AlertCircle className="text-red-500 dark:text-red-400 shrink-0 mt-0.5" size={24} />
              <div>
                <h3 className="text-base font-serif font-semibold text-red-800 dark:text-red-400 tracking-wide">
                  Expired Subscriptions
                </h3>
                <p className="text-sm text-red-700 dark:text-red-200/80 mt-1">
                  You have {expiredSubs.length} subscription{expiredSubs.length > 1 ? 's' : ''} that {expiredSubs.length > 1 ? 'have' : 'has'} expired. Please review them.
                </p>
              </div>
            </div>
          )}
          ...
          {expiringSoon.length > 0 && (
            <div className="bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 rounded-2xl p-5 flex items-start gap-4 shadow-lg shadow-amber-500/5 backdrop-blur-sm transition-colors duration-300">
          ...
          <>
            <div className="mb-8 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md p-5 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm dark:shadow-xl transition-colors duration-300">
              <div className="flex flex-wrap items-center gap-5">
                {viewMode !== 'calendar' && filteredSubscriptions.length > 0 && (
                  <div className="flex items-center gap-3 pr-5 border-r border-zinc-200 dark:border-zinc-700">
                    <button
                      onClick={handleSelectAll}
                      className={`w-5 h-5 rounded flex items-center justify-center transition-colors border ${selectedIds.length > 0 && selectedIds.length === filteredSubscriptions.length ? 'bg-amber-500 border-amber-500 text-zinc-950 dark:bg-amber-400 dark:border-amber-400' : 'border-zinc-300 dark:border-zinc-600 hover:border-amber-500/50 text-transparent'}`}
                      title="Select All"
                    >
                      <Check size={14} strokeWidth={3} />
                    </button>
                    <span className="text-sm font-medium text-zinc-600 dark:text-zinc-400">Select All</span>
                  </div>
                )}
      ...
      {selectedIds.length > 0 && (
        <div className="fixed bottom-8 left-1/2 transform -translate-x-1/2 bg-zinc-900/95 dark:bg-zinc-800/95 backdrop-blur-md border border-zinc-700 dark:border-zinc-600 shadow-2xl shadow-black/50 rounded-2xl px-6 py-3 flex items-center gap-6 z-50 animate-in slide-in-from-bottom-10">
          <span className="text-sm font-medium text-zinc-200">
            <span className="text-amber-400 font-bold">{selectedIds.length}</span> selected
          </span>
          <div className="w-px h-6 bg-zinc-700"></div>
          <div className="flex items-center gap-2">
            <button
              onClick={handleClearSelection}
              className="text-sm font-medium text-zinc-400 hover:text-zinc-200 transition-colors px-3 py-1.5 rounded-lg hover:bg-zinc-700"
            >
              Cancel
            </button>
            <button
              onClick={handleBulkDelete}
              className="flex items-center gap-2 text-sm font-bold text-red-500 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 px-4 py-1.5 rounded-lg transition-colors"
            >
              <Trash2 size={16} />
              Delete
            </button>
